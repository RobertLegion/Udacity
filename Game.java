...
public class GAME2 {
    ...
    public boolean checkPlayer(char player){
        for (int j = 0; j < 3; j++) {
            if (gridAt(0,j) == player && gridAt(1,j) == player && gridAt(2,j) == player)
                 return true; // dla osi x
        }
        for (int i = 0; i < 3; i++) {
            if (gridAt(i,0) == player && gridAt(i,1) == player && gridAt(i,2) == player)
                return true; // dla osi y
        }
        if (gridAt(0,0) == player && gridAt(1,1) == player && gridAt(2,2) == player)
            return true; // przekatne
        if (gridAt(2,0) == player && gridAt(1,1) == player && gridAt(0,2) == player)
            return true; // przekatne
